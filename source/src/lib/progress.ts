...
export function recordQuizAttempt(previous: ChapterQuizProgress, score: number, total: number): ChapterQuizProgress {
  const previousRate = previous.total > 0 ? previous.bestScore / previous.total : 0
  const nextRate = total > 0 ? score / total : 0
  return nextRate > previousRate
    ? { attempts: previous.attempts + 1, bestScore: score, total }
