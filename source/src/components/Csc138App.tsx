...
function Reference({ chapter, selectChapter, reset }: { chapter: Csc138Chapter; selectChapter: (chapter: Csc138ChapterId) => void; reset: () => void }) {
  ...
  return <div className="page reference-page">
    ...
    {isChapter2 && <section className="p2p-reference"><article><span>CLIENT-SERVER DISTRIBUTION</span><strong>D<sub>CS</sub> &gt;= max(NF/u<sub>s</sub>, F/d<sub>min</sub>)</strong><p>One server must emit N copies, and every receiver is limited by its own download capacity.</p></article><article><span>P2P DISTRIBUTION</span><strong>D<sub>P2P</sub> &gt;= max(F/u<sub>s</sub>, F/d<sub>min</sub>, NF/(u<sub>s</sub> + sum u<sub>i</sub>))</strong><p>Peers add upload capacity as they add demand. These formulas are ideal lower bounds.</p></article><article><span>BITTORRENT STRATEGY</span><p><b>Tracker</b> discovers peers · <b>rarest first</b> protects availability · <b>unchoking</b> selects upload partners · <b>optimistic unchoke</b> explores a new partner</p></article></section>}
